    ...
    func loadMaterials() throws -> [WavefrontMaterial] {
        while fileReader.contentAvailable {
            ...
            case .ambientMap:
                ...
                currentMaterial!.ambientMap = try? Texture2D(resourceName: ambientMapName as String)
            case .diffuseMap:
                let diffuseMapName = try fileReader.readToken(errorMessage: "Unable to read diffuse texture file name")
                currentMaterial!.diffuseMap = try? Texture2D(resourceName: diffuseMapName as String)
            case .specularMap:
                let specularMapName = try fileReader.readToken(errorMessage: "Unable to read specular texture file name")
                currentMaterial!.specularMap = try? Texture2D(resourceName: specularMapName as String)
            case .specularExponentMap: // a.k.a specular highlight map
                ...
                currentMaterial!.specularExponentMap = try? Texture2D(resourceName: specularExponentMapName as String)
            case .alphaMap:
                let alphaMapName = try fileReader.readToken(errorMessage: "Unable to read alpha texture file name")
                currentMaterial!.alphaMap = try? Texture2D(resourceName: alphaMapName as String)
            case .bumpMap:
                let bumpMapName = try fileReader.readToken(errorMessage: "Unable to read bump texture file name")
                currentMaterial!.bumpMap = try? Texture2D(resourceName: bumpMapName as String)
            case .displacementMap:
                let displacementMapName = try fileReader.readToken(errorMessage: "Unable to displacement texture file name")
                currentMaterial!.displacementMap = try? Texture2D(resourceName: displacementMapName as String)
            case .stencilDecal:
                let stencilDecalTextureName = try fileReader.readToken(errorMessage: "Unable to stencil decal texture file name")
                currentMaterial!.stencilDecalMap = try? Texture2D(resourceName: stencilDecalTextureName as String)
            }
