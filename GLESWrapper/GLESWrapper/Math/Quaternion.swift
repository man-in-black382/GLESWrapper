...
//  GLESWrapper
//
//  Created by Pavlo Muratov on 05.01.17.
...
//    
    ...
    public static func dotProduct(first: Quaternion, second: Quaternion) -> Float {
        return first.w * second.w + first.x * second.x + first.y * second.y + first.z * second.z;
    }
    
    public static func slerp(first: Quaternion, second: Quaternion, t: Float) -> Quaternion {
        
        // "Cosine of the angle" between 2 quaternions
        var cosOmega = Quaternion.dotProduct(first: first, second: second)
        var second = second
        ...
        return Quaternion(w: first.w * k0 + second.w * k1,
    ...
    public func exponentiated(exponent: Float) -> Quaternion {
        // Check for an identity quaternion
        guard w >= 0.9999 else {
            return self
        }
        ...
        return Quaternion(w: cosf(newAlpha), x: x * relation, y: y * relation, z: z * relation)
    }
