...
class LightingViewController: GLKViewController {
    ...
    var textureArray: Texture2DArray!
...
//        sampleFramebuffer.attach(texture: try! Texture2D(size: CGSize(width: 100, height: 100)), to: .attachment0)
        ...
        textureArray = try! Texture2DArray(size: view.bounds.size, capacity: 3)
    }
    
    override func glkView(_ view: GLKView, drawIn rect: CGRect) {
        try! textureArray.updateTexture(at: 2, withContentsOf: view)
        

//        renderer.renderOffscreen(framebuffer: gradientFramebuffer, configuration: nil)
